...
# This creates a row for each buffer size plus one for colorbars.
hratios = ([1,]*len(buffers))+ [0.3,]

# Create a figure and a grid of subplots for visualizing watershed metrics for all buffers.
f, ax = plt.subplots(len(buffers)+1, 3, figsize=(12, 20), gridspec_kw={'height_ratios': hratios})

# Iterate through all buffer sizes to plot watershed metrics.
for i, buffer in enumerate(buffers + [0,]): # The [0,] is a placeholder to ensure the loop runs for the correct number of rows.
    if i == len(buffers): # Skip the last iteration which is just the placeholder.
        continue 
    ...
    # Set x-axis labels only for the row before the colorbar row (index 3 in the original script, but here it's dynamic).
    # This condition needs to be adjusted to apply to the last actual data row.
    if i == len(buffers) - 1: # Apply to the last data row.
...
caxes = [
    f.add_axes([0.12,0.03,0.2,0.01]),
    f.add_axes([0.43,0.03,0.2,0.01]),
    f.add_axes([0.74,0.03,0.2,0.01])
]
...
plt.savefig(f'figures/buffer_watershed_metrics_dissertation.png', dpi=300) # Save the figure.
