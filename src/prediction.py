...
# Iterate over a list of buffer sizes for prediction.
# The original code had a loop for multiple buffers, but was commented to only run for 50km.
# Keeping the loop structure for flexibility.
for buffer in [50,]: # [0, 1, 2, 5, 10, 20, 50]:
    # Load the FIER scaling image collection for the current buffer size.
    fier_collection = ee.ImageCollection(f'projects/byu-hydroinformatics-gee/assets/kmarkert/fier/nwm/fier_scaling_buffer_{buffer:02d}km')
    # Get a list of HUC8 IDs present in the FIER collection.
    hydroids = fier_collection.aggregate_array('huc8Id').getInfo()

    # Define the directory containing REOF (Reconstructed Empirical Orthogonal Functions) datasets.
    ...
    # Define the directory containing water fraction datasets.
    waterfraction_dir = Path(f'/Users/kelmarkert/Documents/byu/phd/fierarama/water_fractions/buffer_{buffer:02d}km/')
    ...
    # The original code had a slice `[:1]` to process only the first date.
    # Keeping it for demonstration, but typically this would process all dates.
    for date in dates[:1]:
        ...
        for hydroid in hydroids:
            ...
            reach_q = q_date.sel(feature_id=reaches).rename({'feature_id':'reachid'})
            ...
            # If no simulated PCs are generated for a watershed, skip it.
        ...
        # Mosaic all reconstructed images and set the system:time_start property.
        pred = ee.ImageCollection.fromImages(imgs).mosaic().set('system:time_start', ee.Date(date_str).millis())
        ...
        OUTCOLLECTION = f'projects/byu-hydroinformatics-gee/assets/kmarkert/fier/nwm/predictions/fier_buffer_{buffer:02d}km'
        ...
        # export.start() # Uncomment to start the export task.
        
        # break # Uncomment to break after the first date (for testing).
