...
# Create a figure and a grid of subplots for visualizing watershed metrics for the first 4 buffers.
f, ax = plt.subplots(len(buffers[:4])+1, 3, figsize=(10, 12), gridspec_kw={'height_ratios': hratios})

# Iterate through the first 4 buffer sizes to plot watershed metrics.
for i, buffer in enumerate(buffers[:4]):
    # Plot number of RSMs.
    ...
    # Set x-axis labels only for the last row of plots.
    if i == 3:
...
for i in range(3):
    ax[0, i].set_title(titles[i], loc='left') # Set title for the first row of plots.
...
plt.savefig(f'figures/buffer_watershed_metrics_1.png', dpi=300) # Save the figure.
